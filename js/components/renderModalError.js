...
export default function renderModalError() {
    ...
    const iconError = getIconEl("modal_iconError", "44", "44", '<use xlink:href="images/sprite.svg#icon-checkError"></use>')
    iconError.style.cssText = 'width: 44px; height: 44px;';
    ...
    const modalContent = getContainerEl("modal__content");
    ...
    const modalTitle = getTitleEl("modal__title", "Не удалось отправить обращение");
    const modalText = getTextEl("modal__text", "Что-то пошло не так, попробуйте отправить форму еще раз. Если ошибка повторится — свяжитесь со службой поддержки.");

    modalContent.append(iconClose, iconError, modalTitle, modalText);
